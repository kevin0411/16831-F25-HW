    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        for i in range(self.num_grad_steps_per_target_update * self.num_target_updates):
            ...
            if i % self.num_grad_steps_per_target_update == 0:
                ...
                q_target = reward_n + self.gamma * v_s_next * (1 - terminal_n)
                q_target = q_target.detach()

            # every time, update this critic using the observations and targets
            self.optimizer.zero_grad()
            loss = self.loss(self.critic_network(ob_no).squeeze(1), q_target)
            loss.backward()
            self.optimizer.step()
            # HINT: don't forget to use terminal to cut off the V(s') (ie set it
            #       to 0) when a terminal state is reached
            # HINT: make sure to squeeze the output of the critic_network to ensure
            #       that its dimensions match the rewards
