...
class PGAgent(BaseAgent):
    ...
    def train(self, observations, actions, rewards_list, next_observations, terminals):
        ...
        # Compute Q values with helper function
        q_values = self.calculate_q_vals(rewards_list)
        # Compute Advantages with helper function
        advantages = self.estimate_advantage(observations, rewards_list, q_values, terminals)
        # Use the actor to update the policy and get the train log
    ...
    def estimate_advantage(self, obs, rewards_list, q_values, terminals):
        ...
        if self.nn_baseline:
            ...
            # Unnormalize the values to have the same mean and std as q_values
            ...
            if self.gae_lambda is not None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    ## HINT 2: self.gae_lambda is the lambda value in the
                        ## GAE formula
                    # Determine if it's the end of a trajectory
                    if terminals[i]:
        ...
        if self.standardize_advantages:
            ...
            ## Simply subtract the mean from the advantages to get a mean of zero
            advantages = (advantages - advantages.mean())
            ## Divide the advantages by the standard deviation to get a standard deviation of one
    ...
    def _discounted_return(self, rewards):
        ...
        T = len(rewards)
        # Compute the total discounted reward for the entire trajectory
        discounted_value = sum(self.gamma**(t_prime) * rewards[t_prime] for t_prime in range(0, T))
        # Assign this value to all timesteps
        for t in range(T):
            discounted_returns[t] = discounted_value
    ...
    def _discounted_cumsum(self, rewards):
        ...
        T = len(rewards)
        # Compute the discounted cumulative sum for each timestep
        for t in range(T):
            # Compute the sum of discounted rewards from time t to the end
            discounted_cumsums[t] = sum(self.gamma**(t_prime - t) * rewards[t_prime] for t_prime in range(t, T))
